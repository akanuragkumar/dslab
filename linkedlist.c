...
main ()
{
  ...
  while (1)
    {
      printf	("\n1. Insert front 2.Insert rear. 3. Delete front 4. Delete rear 5. Insert at postion\n6. Delete at postion 7.Delete based on value 8. Display 9. Display reverse 10. Reverse list 11. Exit\n");
	...
	case 2:
	  ...
	  break;
	case 3:
	  head = delet_front (head, &y);
	  if (y != -1)
	    printf ("deleted item is %d\n", y);
	  break;
	case 4:
	  head = delet_rear (head, &y);
	  if (y != -1)
	    printf ("deleted item is %d\n", y);
	  break;
	case 5:
...
// recursive function to traverse reverseNODE 
...
NODE
del_pos (NODE head, int *val, int pos)
{
  NODE tmp, cur;
  int k = 1;
  tmp = cur = head;
...
NODE
del_val (NODE head, int *val, int key)
{
  NODE tmp, cur, p;
  tmp = cur = head;
  *val = -1;
